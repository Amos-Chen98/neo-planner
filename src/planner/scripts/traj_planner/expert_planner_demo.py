...
class MinJerkPlanner(TrajUtils):
    ...
    def __init__(self, config=DefaultConfig()):
        ...
        self.plan_attempt = [PlanAttempt() for _ in range(self.batch_num)]
    ...
    def batch_plan(self, map, head_state, tail_state):
        ...
        for i in range(self.batch_num):
            self.plan_attempt[i].init_pos_array = []
            self.plan_attempt[i].opt_pos_array = []
            try:
                self.read_planning_conditions(map, head_state, tail_state, batch_init_wpts[i], ts)
                # record init conditions
                self.plan_attempt[i].init_wpts = batch_init_wpts[i]
                self.plan_attempt[i].init_ts = ts
                self.plan_attempt[i].init_pos_array = self.get_pos_array()
                ...
                print(f"Computation time[{i}] = {computation_time}")

                # record opt wpts and ts
                self.plan_attempt[i].opt_wpts = self.int_wpts
                self.plan_attempt[i].opt_ts = self.ts
                self.plan_attempt[i].opt_pos_array = self.get_pos_array()
