...
LastEditTime: 2024-03-03 17:03:38
'''
import os
import sys
...
from traj_planner.neo_planner import NeoPlanner
...
class TrajPlanner():
    def __init__(self, node_name="traj_planner"):
        ...
        self.selected_planner = rospy.get_param("~selected_planner", 'basic')  # 'basic', 'batch', 'expert', 'record', 'nn', or 'neo'
        ...
        self.global_target_sub = rospy.Subscriber('/move_base_simple/goal', PoseStamped, self.global_target_cb)
    ...
    def global_target_cb(self, data):
        self.target_received = True
        self.global_target = np.array([data.pose.position.x, data.pose.position.y])
    ...
    def try_local_planning(self):
        ...
        while True:
            try:
                ...
                rospy.logwarn("Retrying for: %s", ex)
                seed += 1
                self.set_local_target(seed)
                if seed > 10:
                    rospy.logwarn("Local replanning failed!\n")
                    return
